    ...
    <div className="fixed bg-gradient-to-b from-[#005BEA] to-[#00C6FB] bg-fixed w-full h-screen z-[-100]">
      ...
      <div className="w-full md:w-1/3 mx-auto md:px-0 px-4 mt-4">
        <div className="flex justify-center items-center flex-col space-y-8">
          <h2 className="text-lg font-semibold text-white">Log in to your account</h2> 
          ...
          <div className="w-full">
            ...
            <div className="flex justify-end items-center w-full">
              <span onClick={() => {}} className="text-xs text-white">Forgotten password?</span>
            </div>
          </div>
          ...
          <div className="w-full">
            ...
            <button className="text-xs text-primary font-semibold bg-white p-2 w-full rounded-lg" disabled={loading} onClick={handleClick}>Log In</button> {/**mt-4 self-center */}
            {err && <span className="text-xs text-rose-400 mt-3">{err.message}</span>}
          </div>

          <span className="text-sm mt-2 text-dimWhite">Don&apos;t have an account? <button type="button" onClick={() => {}} className="text-white outline-0 border-0">Sign up</button> </span>
        </div>
      </div>
    </div>
  )
